...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{fig_5_1_steady_state_architecture.pdf}
    \caption{Steady-state system architecture pipeline illustrating nominal processing loops, edge perception, low-level telemetry, and P2P decentralized coordination layers.}
    \label{fig:steady_state_architecture}
\end{figure}
...
\end{equation}
Given the sequence of updates, the state vector of the system at the $k$-th update period $t_k \in \mathcal{T}_{\text{AMCL}}$ is represented as the planar position vector in Equation~\eqref{eq:amcl_pose_vector}:
\begin{equation}
...
Multi-agent fleet management typically relies on centralized dispatch servers that allocate trajectories and resolve spatial conflicts~\cite{gupta2025robotfleet}. While such approaches are typically computationally light, they have a single point of failure and require uninterrupted communications from every robot in the fleet to the master controller. In practice, this is a questionable assumption in large-scale warehouse environments where deep steel shelves create wireless dead zones and multi-path fading. A central controller cannot safely arbitrate collisions in a steel-enclosed aisle if one of the robots in it has lost radio contact. Our decentralized P2P coordination approach breaks this centralized single point of failure by pushing the arbitration logic into the agents. Acting as independent edge nodes, the robots coordinate their movements on the industrial floor utilizing a decentralized, brokerless data distribution paradigm native to the fleet's coordinated communication framework \cite{macenski2022robot}. 

\subsection{Corridor Spatial Mutex Formulations and Shared Corridors}

The central problem of a decentralized approach is preventing deadlocks in confined spaces. Whereas most open regions allow for sufficient lateral space for two robots to safely pass each other, narrow aisles have insufficient width for such a maneuver. Specifically, the narrow aisle width was found to be less than twice the robot's safety radius plus an epsilon-long robot-specific margin. Therefore, the local costmaps of robots approaching each other from opposite ends of the aisle would erroneously inflate their respective walls and the other robot's body, making mutual passage impossible.

To formalize this idea into an algorithm, we define the narrow aisle spatial mutex at the corridor level instead of the inspection region level. There are 12 narrow shared corridors in the factory layout; 3 main longitudinal corridors are between the high-rise shelves and 9 narrow ones are between the facing small retail shelves. Every narrow aisle is a shared workspace between 2 parallel storage faces, which requires mutual exclusion to prevent the inter-agent collisions, local costmap inflation, and resulting deadlocks. By contrast, the virtual mobile cluster is placed at the wide loading dock, which allows the robots to pass from both sides of the aisle. As a result, the task allocator does not impose the mutual exclusion of the shared corridor on the mobile robot cluster.

To avoid the spatial deadlocks of the narrow transit zones, the workspace is divided into local regions of mutual exclusion for the robotic fleet. The high-rise storage area is divided into 3 primary longitudinal corridors along the north-south axis. The western corridor is located between the positive x-face of the first big shelf (\texttt{shelf\_big\_0\_xplus}) and the negative x-face of the fifth big shelf (\texttt{shelf\_big\_4\_xminus}). The middle longitudinal corridor is between the positive x-face of the fifth big shelf (\texttt{shelf\_big\_4\_xplus}) and the negative x-face of the fourth big shelf (\texttt{shelf\_big\_3\_xminus}). Finally, the eastern longitudinal corridor is between the positive x-face of the fourth big shelf (\texttt{shelf\_big\_3\_xplus}) and the negative x-face of the third big shelf (\texttt{shelf\_big\_2\_xminus}).

Further, the same principle is applied to the small shelf storage area to define 9 narrow aisles between the facing y-faces of the small shelves, distributed across three distinct structural columns. On the western column of small shelves, the top narrow aisle is between \texttt{shelf\_2\_yminus} and \texttt{shelf\_1\_yplus}, the middle narrow aisle is between \texttt{shelf\_1\_yminus} and \texttt{\seqsplit{shelf\_0\_yplus}}, and the bottom narrow aisle is between \texttt{shelf\_0\_yminus} and \texttt{\seqsplit{shelf\_yplus}}. Similarly, on the eastern column of small shelves, the top narrow aisle is between \texttt{shelf\_3\_yminus} and \texttt{shelf\_4\_yplus}, the middle narrow aisle is between \texttt{shelf\_4\_yminus} and \texttt{shelf\_6\_yplus}, and the bottom narrow aisle is between \texttt{shelf\_6\_yminus} and \texttt{\seqsplit{shelf\_5\_yplus}}. Finally, on the southeast corner column of small shelves, the top narrow aisle is located between \texttt{shelf\_10\_yminus} and \texttt{shelf\_9\_yplus}, the middle narrow aisle is between \texttt{shelf\_9\_yminus} and \texttt{shelf\_8\_yplus}, and the bottom narrow aisle is established between \texttt{shelf\_8\_yminus} and \texttt{shelf\_7\_yplus}. The coordination module mutually excludes all narrow aisles for the robotic fleet. This way, the local mutual exclusion primitive of the P2P navigation is applied to the entire length of every narrow aisle, which successfully avoids the spatial deadlocks of the narrow transit zones \cite{ricart1981optimal}. The global physical constants, shelving dimensions, and narrow aisle mutual exclusion mappings governing this spatial partition are compiled in Appendix~\ref{app:code_config} (Listing~\ref{lst:config_blueprints_core}).

\subsection{Peer-to-Peer Claims Protocol and Lease Mechanics\label{sec:ch5_claims}}
...
\begin{equation}
\thinmuskip=1mu\medmuskip=2mu\thickmuskip=2mu
...
\end{equation}
which imposes a linear ordering of the packets by their arrival time and agent-specific identifier. Practically however, it is impossible to enforce such ordering on the edge due to serialization overhead on the network. The physical peer-to-peer stack uses a stateless local runtime coordination mechanism, which leverages LCFS (Last-Come-First-Served) lease preemption. By doing so, incoming lease updates can preemptively overwrite local lock registries~\cite{coskun2021deadlock}, which ultimately impacts the timestamp measurements recorded in \texttt{anomaly\_latency.csv}, obviating the need to maintain a queue at the edge that would serialize updates on the planar manifold, conserving critical edge compute cycles and reducing coordination induced latency(see Figure~\ref{fig:corridor_lease_sequence}.). The complete Python operationalization of this lease-renewal cycle, LCFS overwrite arbitration, and blacklist propagation logic is implemented within the \texttt{ClaimManager} class, as compiled in Appendix~\ref{app:code_claim_manager} (Listing~\ref{lst:claim_manager_core}). Figure~\ref{fig:ch5_claim_terminal} shows the resulting claim registry as reported by the passive diagnostic terminal during an active mission, listing the side-key currently locked by each of the three agents alongside the fleet-wide progress summary.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_5_2_5_diagnosis_claim_view}
\caption{Terminal output of \texttt{system\_diagnosis} showing the active corridor claims held by \texttt{robot1}, \texttt{robot2}, and \texttt{robot3} at a given mission timestamp, together with per-robot pose, battery, and completed-waypoint telemetry.}
\label{fig:ch5_claim_terminal}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{fig_5_2_corridor_lease_sequence.pdf}
\caption{Peer-to-peer corridor claim and lease-renewal protocol between two agents, illustrating the asynchronous claim broadcast (Eq.~\ref{eq:spatial_availability_check}), the LCFS tie-breaking rule (Eq.~\ref{eq:asymmetric_precedence}), and the $T_{\text{renew}}=1.5\,\text{s}$ lease-renewal cycle.}
\label{fig:corridor_lease_sequence}
\end{figure}
...
\label{eq:projected_pose_vector}
\end{equation}
...
\end{equation}
By formulating the homogeneous transformations for local offset, rotation, and robot-specific transformation in this manner, the spatial parser node guarantees kinematic compatibility of the subsequent approach pose and camera transform calculations relative to the non-holonomic constraints of the agents. The software routines executing these planar matrix operations, including the CAD origin offset transformations and local-to-global frame mappings, are defined in the SDF geometry engine detailed in Appendix~\ref{app:code_sdf_parser} (Listing~\ref{lst:sdf_transform_core}).

\subsection{Parametric Approach Pose Generation and Steel-Column Structural Bypass}\label{sec:ch5_steel_bypass}
...
\begin{equation}
\begin{aligned}
\begin{bmatrix} gx_{s, i} \\ gy_{s, i} \end{bmatrix} &= \begin{bmatrix} cx \\ cy \end{bmatrix} + \begin{bmatrix} \cos(\psi_{\text{yaw}}) & -\sin(\psi_{\text{yaw}}) \\ \sin(\psi_{\text{yaw}}) & \cos(\psi_{\text{yaw}}) \end{bmatrix} \\
&\quad \begin{bmatrix} \Delta x_{\text{local, i}} \\ \delta_s \cdot D_{\text{offset}} \end{bmatrix}
\end{aligned}
\label{eq:small_shelf_approach_pose}
\end{equation}
...
\begin{equation}
\begin{aligned}
\begin{bmatrix} gx_{b, i} \\ gy_{b, i} \end{bmatrix} &= \begin{bmatrix} cx \\ cy \end{bmatrix} + \begin{bmatrix} \cos(\psi_{\text{yaw}}) & -\sin(\psi_{\text{yaw}}) \\ \sin(\psi_{\text{yaw}}) & \cos(\psi_{\text{yaw}}) \end{bmatrix} \\
&\quad \begin{bmatrix} \delta_s \cdot D_{\text{offset}} \\ \Delta y_{\text{local, i}} \end{bmatrix}
\end{aligned}
\label{eq:big_shelf_approach_pose}
\end{equation}
...
\end{equation}
from the positive side. Indeed, such a displacement would require the robot to finalize its approach to the inspection section within the inflated collision area of the warehouse's wall. To ensure a collision-free approach to the inventory unit while maintaining the robot's viewpoint normal to the inspection section, the coordinate estimator utilized a modified inspection approach. In particular, while the robot maintained the standard approach offset of $1.81\,\text{m}$ from the negative side, the positive-side approach was redirected with an orthogonal standing offset of: 1.60 m. To maintain strict alignment with the physical configuration space of the fleet, this displacement is calculated as a sum of the half pallet width of 0.40 m (derived from the full model width of 0.80 m defined in the configuration specs) and the nominal approach clearance of 1.20 m, yielding exactly the required 1.60 m clearance. As a result, the approach waypoint provided an unobstructed inspection vector $(x_{\text{approach}}, y_{\text{approach}})$ with an orthogonal viewpoint for the visual inventory check.(Figure~\ref{fig:sdf_waypoint_pipeline} summarizes this transformation chain.) The exact algorithmic implementation of this structural bypass rule, along with the parametric section partitioning for retail and industrial shelving units, is provided in Appendix~\ref{app:code_sdf_parser} (Listing~\ref{lst:sdf_shelving_core}). Figure~\ref{fig:ch5_first_shelf_routes} shows the resulting global plan produced by the runtime SDF parser for one such approach, superimposed on the AMCL-tracked pose and local costmap of the requesting agent inside RViz2.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{fig_5_3_3_rviz2_first_shelf_routes.pdf}
\caption{RViz2 visualization of a global path computed toward a dynamically parsed approach waypoint on \texttt{shelf\_2}, confirming that the coordinates generated by \texttt{sdf\_parser.py} resolve to a reachable, collision-free target inside the live navigation stack.}
\label{fig:ch5_first_shelf_routes}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_5_3_sdf_waypoint_pipeline.pdf}
\caption{Runtime pipeline from SDF world-file parsing to final global approach waypoints, including the SE(2) projection (Eq.~\ref{eq:projected_pose_vector}), homogeneous offset correction (Eq.~\ref{eq:homogeneous_offset_projection}), and the steel-column boundary correction (Eq.~\ref{eq:column_shifted_pos}).}
\label{fig:sdf_waypoint_pipeline}
\end{figure}
...
\subsection{Orientation-Driven Sweeping for Dead-End Wall Mitigation}\label{sec:ch5_deadend_sweep}
...
\end{equation}
This preprocessing reduces the input dimensionality while eliminating most of the high-frequency components of the original image. Next, the system extracts micro-texture and macro-structural features using two separate feature encoding pipelines~\cite{liu2023tire}.
...
To utilize these descriptors in an edge-deployed anomaly classifier, micro and macro representations can be joined together to form a fusion feature using the process of concatenation of two vectors \cite{liu2023tire} as shown below:
...
\end{equation}
and the resulting intensity values are clipped to the 0-255 range. This final transformation is critical in endowing the vision module with the ability to deal with extreme lighting conditions, and thus it is applied right before the LBP descriptor is extracted from the image \cite{ojala2002multiresolution}.
...
The cardboard anomaly classification task on the edge is particularly challenging due to the need to reach a high-confidence decision at the fastest possible time. To this end, the reduced feature vector representation is scored using a soft-margin Linear Support Vector Machine~\cite{cortes1995support}. Instead of directly optimizing the primal problem, which for the case of high-dimensional input bounds can scale poorly, the optimal separating hyperplane is resolved in the dual formulation as:
\begin{equation}
...
While being effective in automated cardboard packaging inspection, such an approach lacks the ability to offer visual explanations of the decisions to the human supervisor, thus potentially undermining the overall cyber-physical system's trustworthiness~\cite{samek2017explainable}. The common model-agnostic post-hoc explanation techniques, such as input perturbation, require an unacceptable amount of computational resources for the embedded robotic vision application. To resolve the conundrum, the interpretability is baked into the model by leveraging the linearity of the reduced-space classifier's decision boundary.
...
\end{equation}
where the blending coefficient alpha is set to $\alpha_{\text{blend}} = 0.35$. Notably, the full feature-space attribution pipeline runs on the robotic edge vision system in only 1.2 ms, thus enabling real-time attention-aware robotic box inspection.(The complete pipeline, from raw frame to XAI overlay, is depicted in Figure~\ref{fig:perception_xai_pipeline}.) The mathematical back-projection pipeline converting SVM hyperplanes into $4 \times 4$ spatial grid attributions and colormap overlays is programmatically realized in Appendix~\ref{app:code_xai} (Listing~\ref{lst:xai_backproject_core}). Figure~\ref{fig:ch5_xai_live} shows this overlay as rendered live inside the operator dashboard during an active anomaly classification event, confirming that the closed-form projection of Equation~\ref{eq:alpha_blending} executes within the perception thread without visibly interrupting the camera stream.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.55\textwidth]{fig_5_6_3_ui_fault_detection.pdf}
\caption{Operator dashboard displaying a live JET-colormap XAI overlay on the \texttt{robot3} camera feed following a \texttt{damaged} classification, alongside the corresponding anomaly alert entry in the system log.}
\label{fig:ch5_xai_live}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig_5_6_perception_xai_pipeline.pdf}
\caption{End-to-end perception and classification pipeline: LBP/HOG feature fusion (Eq.~\ref{eq:hog_total_dim}), PCA projection, linear SVM decision (Eq.~\ref{eq:svm_decision_function}), and closed-form spatial back-projection into the final XAI attention overlay (Eq.~\ref{eq:alpha_blending}).}
\label{fig:perception_xai_pipeline}
\end{figure}
